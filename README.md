...
管理者ページに、勉強会の発表者・テーマ・紹介ツール・評価（★1〜5＋コメント）を記録する「発表者管理」カードがあります。発表者ごとの登壇回数・平均評価を自動集計し、履歴として蓄積します。閲覧・編集とも管理者のみ（Firestore `aiStudy_presentations` コレクション）。

過去のスプレッドシート「AI勉強会 発表者まとめ」の内容（第1〜4回）は、管理画面の「スプレッドシートの過去データを取り込む」ボタンから一度だけ取り込めます。
